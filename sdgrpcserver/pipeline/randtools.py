...
def batched_rand(
    ...
    if shape[0] % len(generators) != 0:
        raise ValueError(f"shape[0] ({shape[0]}) needs to be a multiple of len(generators) ({len(generators)})")

    latents = torch.cat([
        torch.rand((1, *shape[1:]), generator=generator, device=generator.device, dtype=dtype) 
...
class TorchRandOverride:
    ...
    def randn_like(self, input: torch.Tensor, *args, dtype: Optional[torch.dtype]=None, device=None, **kwargs):
    ...
    def randint_like(self, input, *args, high=None, low=None, dtype=None, layout=torch.strided, device=None, **kwargs):
        ...
        latents = torch.cat([
            torch.randint(size=(1, *input.shape[1:]), low=low, high=high, generator=generator, device=generator.device, dtype=dtype) 
